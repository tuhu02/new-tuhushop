    ...
    return (
        <AdminLayout title="Transaksi Manual" headerTitle="Transaksi Manual">
            <div className="space-y-6 p-4 lg:p-6">
                ...
                {/* Tab Filter */}
                <div className="flex gap-1 rounded-lg border border-sidebar-border/70 bg-muted/30 p-1 w-fit">
                    {tabs.map((tab) => {
                        const Icon = tab.icon;
                ...
                <Card>
                    <CardHeader className="pb-3">
                        <CardTitle className="text-base">
                            {filters.status === 'pending' && 'Transaksi Menunggu Proses'}
                            {filters.status === 'done' && 'Transaksi Selesai Diproses'}
                            {filters.status === 'all' && 'Semua Transaksi Manual'}
                        </CardTitle>
                        <CardDescription>
                            ...
                            <table className="w-full min-w-225 text-left text-sm">
                                ...
                                <tbody>
                                    ...
                                    {transactions.data.map((tx) => (
                                        <tr
                                            key={tx.id}
                                            className="border-t border-sidebar-border/40 hover:bg-muted/20 transition-colors"
                                        >
                                            {/* Referensi */}
                                            <td className="px-4 py-3">
                                                <div className="font-mono text-xs text-primary">
                                                    {tx.reference || tx.merchant_ref}
                                                </div>
                                            ...
                                            <td className="px-4 py-3">
                                                ...
                                                {tx.customer_inputs && Object.keys(tx.customer_inputs).length > 0 && (
                                                    <div className="mt-1 space-y-0.5">
                                                        {Object.entries(tx.customer_inputs).map(([k, v]) => (
                                            ...
                                            <td className="px-4 py-3">
                                                <StatusBadge
                                                    status={tx.status}
                                                    digiflazzStatus={tx.digiflazz_status}
                                                />
                                                {tx.digiflazz_sn && (
                                                    <div className="mt-1 text-xs text-green-700 bg-green-50 rounded px-1.5 py-0.5 inline-flex items-center gap-1">
                                            ...
                                            <td className="px-4 py-3 text-xs text-muted-foreground whitespace-nowrap">
                                                ...
                                                {tx.digiflazz_processed_at && (
                                                    <div className="text-green-600 mt-0.5">
                                                        ✓ {formatDate(tx.digiflazz_processed_at)}
                                                    </div>
                                                )}
                                            </td>
                                            ...
                                            <td className="px-4 py-3">
                                                ...
                                                ) : tx.digiflazz_status === 'Sukses' || tx.digiflazz_status === 'sukses' ? (
                        ...
                        {transactions.last_page > 1 && (
                            <div className="flex items-center justify-center gap-1 border-t border-sidebar-border/50 p-4">
                                {transactions.links.map((link, i) => (
                                    <Link
                                        key={i}
                                        href={link.url ? `${link.url}&status=${filters.status}` : '#'}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div className="relative z-10 mx-4 w-full max-w-lg rounded-xl border border-sidebar-border bg-background shadow-2xl">
                        ...
                        <div className="flex items-center justify-between border-b border-sidebar-border/50 px-6 py-4">
                            <div>
                                <h2 className="text-lg font-semibold">Proses Transaksi Manual</h2>
                        ...
                        <div className="px-6 py-5 space-y-4">
                            {/* Transaction Info */}
                            <div className="rounded-lg border border-sidebar-border/50 bg-muted/30 p-4 space-y-2.5">
                                <InfoRow icon={Hash} label="Referensi" value={modalTx.reference || modalTx.merchant_ref} mono />
                                <InfoRow icon={User} label="Customer" value={modalTx.customer_name || '-'} />
                                <InfoRow icon={Phone} label="No. HP" value={modalTx.customer_phone || '-'} />
                                <InfoRow icon={Package} label="Produk" value={`${modalTx.product?.name || '-'} — ${modalTx.product_price?.display_name || '-'}`} />
                                <InfoRow icon={CreditCard} label="Target" value={modalTx.digiflazz_customer_no || '-'} mono />
                                {modalTx.customer_inputs && Object.entries(modalTx.customer_inputs).map(([k, v]) => (
                                    <InfoRow key={k} icon={FileText} label={k.charAt(0).toUpperCase() + k.slice(1)} value={v} />
                                ))}
                            </div>

                            {/* Serial Number Input */}
                            <div className="space-y-1.5">
                                <label className="text-sm font-medium" htmlFor="serial_number">
                                    Serial Number / Akun <span className="text-red-500">*</span>
                                </label>
                                <textarea
                                    id="serial_number"
                                    rows={3}
                                    className="w-full rounded-lg border border-sidebar-border/70 bg-background px-3 py-2 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-primary/50 resize-none placeholder:text-muted-foreground"
                                    placeholder="Contoh: user@email.com | password123&#10;atau kode voucher: ABCD-1234-EFGH"
                                    value={serialNumber}
                                    onChange={(e) => setSerialNumber(e.target.value)}
                                />
                                <p className="text-xs text-muted-foreground">
                                    Isi dengan akun, kode voucher, atau serial number yang akan dikirimkan ke customer.
                                </p>
                            </div>
                            ...
                            <div className="space-y-1.5">
                                ...
                                <textarea
                                    id="notes"
                                    rows={2}
                                    className="w-full rounded-lg border border-sidebar-border/70 bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50 resize-none placeholder:text-muted-foreground"
